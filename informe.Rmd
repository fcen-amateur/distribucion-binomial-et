...
$$
\frac{1}{n}{\sum_{i=1}^{n}{g(X_i)}}=\mathbb{E}(g(X_1) \ | \ \theta = \hat{\theta}) = \mathbb{E}_\hat{\theta}}(g(X_1))
$$
...
$$
\begin{aligned}
\frac{1}{n}{\sum_{i=1}^{n}{X_i}}&=\mathbb{E}(X_1 \ | \ \theta = (\hat{k}_{m_1},p)) \\
\overline{X} &= \hat{k}_{m_{1}} \times p \\
\hat{k}_{m_1} &= \frac{\overline{X}}{p}
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{1}{n}{\sum_{i=1}^{n}{X_{i}^{2}}}&=E(X_1^2 \ | \ \theta = (\hat{k}_{m_1},p)) = E_{\hat{k}_{m_1}}(X_1^2) \\
\overline{X^2} &=Var_{\hat{k}_{m_1}}(X_1) + [E_{\hat{k}_{m_1}}(X_1)]^2 \\
\overline{X^2} &=\hat{k}_{m_1} \ p \ (1-p) + [\hat{k}_{m_1} \ p]^2 \\
0 &= \hat{k}_{m_1}^{2} \ p^2  + \hat{k}_{m_1} \ p  (1- p) - \overline{X^2} \end{aligned}
$$
Que es una ecuación cuadrática con coeficientes $a=p^2, \ b=p(1-p), \ c = -\overline{X^2}$. La expresión general de sus raíces nos da

$$
\hat{k}_{m_1}^{2} = \frac{p(p-1) \pm \sqrt{p^2(1-p)^2 + 4  p^2  \overline{X^2}}}{2p^2}
$$
Que tomando factor común $p$ en en el numerador y denominador se simplifica a 
$$

\hat{k}_{m_1}^{2} = \frac{p-1 + \sqrt{(1-p)^2 + 4 \overline{X^2}}}{2p}
$$
...
$$
\textbf{p}(\mathbb{X} \ | \ \theta=(\hat{k}_{mv}, p)) = \max_{\theta \\ \in \Theta} \textbf{p}(\mathbb{X} \ | \ \theta)
$$
Al comienzo del trabajo mencionamos que para la familia de distribuciones binomiales, la probabilidad conjunta de la muestra se puede escribir como
$$
\begin{aligned}
\textbf{p}(\mathbb{X} \ | \ \theta) &= \left[\prod_{i=1}^{n}{\binom{k}{x_i}}\right] \times p^{n\overline{x}} \times (1-p)^{n(k - \overline{x})} \\
&= \left( \frac{p}{1-p} \right)^{n\overline{x}} \times  \left[\prod_{i=1}^{n}{\binom{k}{x_i}}\right] \times (1-p)^{nk} \\
&\propto \left[\prod_{i=1}^{n}{\binom{k}{x_i}}\right] \times (1-p)^{nk}

\end{aligned}
$$
Maximizar esta última expresión sobre $k$ es equivalente a maximizar $\textbf{p}(\mathbb{X})$, ya que asumimos que $p$ es conocido. Llamemos a esta última expresión $L(k \ | \ \mathbb{X}, \theta)$, y veamos en qué valor se maximiza. Como la función es discreta, podemos definir el operador diferencia de la función $f$ en el parámetro $x$

$$
\Delta(f,x) = f(x+1, ...) - f(x, ...)
$$
Y estudiar la desigualdad $\Delta(L,k)>0$, es decir, sobre qué intervalo(s) de $k$ es $L$ creciente. Aquí será útil recordar que 

$$
\binom{m+1}{n} = \binom{m}{n} \times \frac{m+1}{m+1-n}
$$
...
\begin{aligned}
 \Delta(L, k) &= L(k+1) - L(k) \\
 &= \left[\prod_{i=1}^{n}{\binom{k+1}{x_i}}\right] \times (1-p)^{n(k+1)} - \left[\prod_{i=1}^{n}{\binom{k}{x_i}}\right] \times (1-p)^{nk} \\
 &= \left[\prod_{i=1}^{n}{\binom{k}{x_i}}\right] \times \left[\prod_{i=1}^{n}{\frac{k + 1}{k + 1 - x_i}}\right]\times (1-p)^{nk} \times (1-p)^{n} - \left[\prod_{i=1}^{n}{\binom{k}{x_i}}\right] \times (1-p)^{nk} \\
 &=\left[\prod_{i=1}^{n}{\binom{k}{x_i}}\right] \times (1-p)^{nk}\times \left[ (1-p)^{n} \times \left[\prod_{i=1}^{n}{\frac{k + 1}{k + 1 - x_i}} \right]- 1 \right]

\end{aligned}
$$
Como $(1-p)^{nk} \times \prod_{i=1}^{n}{\binom{k}{x_i}}$ es positivo cuando $k > 0; \ n > 0; \ 0<p<1$, $\Delta(L, k)$ será positiva mientras $(1-p)^{n} \times \left[\prod_{i=1}^{n}{\frac{k + 1}{k + 1 - x_i}} \right] > 1$:

$$
(1-p)^{n} \times \left[\prod_{i=1}^{n}{\frac{k + 1}{k + 1 - x_i}} \right] > 1 \\
(1-p)^{n} \times \left[\prod_{i=1}^{n}{\frac{k + 1}{k + 1 - x_i}} \right] > 1 \\
n \times \ln(1-p)  + \sum_{i=1}^{n} \ln(k + 1) - \sum_{i=1}^{n} \ln(k + 1 - x_i) > 0
$$
Esta expresión no es propicia a mayores
## De g(tita) = p
